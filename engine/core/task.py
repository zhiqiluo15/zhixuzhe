...
# 执行提示词
EXECUTE_SYSTEM = """执行以下任务步骤。

目标：{goal}
当前步骤：{step}{context}{memory}

严格规则：
- 必须调用工具获取真实信息，禁止编造数据、搜索结果、文件内容、命令输出
- 工具返回什么就汇报什么，不得在结果上添油加醋
- 如果工具调用失败或返回空，如实报告，不要用想象的内容填充
- 完成后用简洁语言如实汇报执行结果"""
...
严格规则：
- 只基于上述步骤结果给出结论，不要编造步骤中没有的信息
- 如果某些步骤失败或信息不足，明确指出，不要用猜测填补
- 给出综合性的最终回答，要具体、有信息量、忠于事实"""
